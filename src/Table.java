...
public class Table<T, R extends RowMapper<T>> {
    ...
    /**
     * Constructs a Table with provided entries and empty columns. Used by the builder to begin table configuration
     * using a provided list entries.
     *
     * @param entries The list of entries to use.
     */
    protected Table(List<T> entries) {
    ...
    protected void setEntries(List<T> entries) {
        if(verifyTable(entries))
            this.entries = entries;
        else
            throw new IllegalArgumentException("The number of entries in the row does not match the number of columns");
    }
    ...
    private boolean verifyTable() {
        return verifyTable(this.entries);
    }
     ...
     * @return True if valid, otherwise false.
     */
    private boolean verifyTable(List<T> entries) {
        if (!entries.isEmpty()) {
            int expectedSize = this.getColumns().size();
            for (int i = 0; i < this.getMappedEntries().size(); i++)
                if (this.getMappedEntries(entries).get(i).size() != expectedSize)
                    return false;
        }
        return true;
    }
    ...
    public static class Builder<T, R extends RowMapper<T>> {
         ...
         * @param entries   The list of entries to be uses by the table.
         */
        public Builder(R rowMapper, List<T> entries) {
            this(new Table<>(entries), rowMapper);
        }
         ...
         */
        protected Builder(Table<T, R> table, R rowMapper) {
            this.table = table;
        ...
        /**
         * Adds a list of entries to the table's current list.
         *
         ...
         * @return The Builder instance for method chaining.
         */
        public Builder<T, R> entries(List<T> entries) {
